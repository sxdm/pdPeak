function[intvls,varargout]=findxxintvl2s(percent,lndata)
%
frck=100;
%
u10=fzero(@(x1)integral(@(x2)posts(x2,lndata),0,x1)-0.9,0);
l=fzero(@(x1)integral(@(x2)posts(x2,lndata),0,x1)-0.001,0);
mx=fzero(@(x1)integral(@(x2)posts(x2,lndata),0,x1)-0.9999,0);
dk=(u10-l)/frck;
k=0:dk:mx;
pdf=posts(k,lndata);
